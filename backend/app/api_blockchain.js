...
router.get('/blocks', JwtMiddleware.checkToken, async (req, res) => {
  res.json(globalVariable.blockchain.chain)
})
...
router.get('/blocks/:id', JwtMiddleware.checkToken, async (req, res) => {
  ...
  var result = globalVariable.blockchain.chain.find((item) => item.timestamp == req.params.id)
  if (result) {
    res.json(result)
  } else {
...
router.post('/mine', JwtMiddleware.checkToken, async (req, res) => {
  try {
    const block = globalVariable.blockchain.addBlock(req.body)
    if (block) {
      console.log(`New block added: ${block.toString()}`)
      globalVariable.pubsub.broadcastChain()
